...
if credentials_file.exists() and client_secrets_file.exists():
    ...
    logger.warning("[API] YouTube OAuth2未配置，文件上传功能可用，但自动上传YouTube不可用")
...
async def upload_video(
    ...
    try:
        ...
        if auto_upload_youtube and youtube_uploader:
            ...
            logger.info(f"[API] 跳过YouTube自动上传（auto_upload_youtube={auto_upload_youtube}, youtube_uploader={'已配置' if youtube_uploader else '未配置'}）")
...
async def upload_batch(
    ...
    try:
        ...
        # 上传视频
        video_result = await upload_video(
            request=request,
            file=video_file,
            title=None,
            description=None,
            tags=None,
            auto_upload_youtube=False  # 先不上传，等脚本上传后再上传
        )
        ...
        script_result = None
        if script_file:
            script_result = await upload_script(
                request=request,
                file=script_file,
                video_filename=video_filename
            )
            ...
            if not script_result.get("success"):
                raise HTTPException(status_code=500, detail="脚本上传失败")

        # 如果配置了自动上传YouTube
        youtube_result = None
        if auto_upload_youtube and youtube_uploader:
            logger.info(f"[API] 开始批量上传到YouTube...")

            video_path = Path(video_result["data"]["video_path"])

            # 上传到YouTube
            youtube_result = youtube_uploader.process_video(video_path)
        ...
        return {
            ...
            "data": {
                ...
                "script": script_result["data"] if script_result else None,
                "youtube_upload": youtube_result
            }
        }
